    ...
    <div className="h-[calc(100vh-64px)] flex flex-col bg-slate-50">
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {['capability', 'prompt & policy', 'suggestions', 'history'].map(panel => (
            <button
              key={panel}
      ...
      <main className="flex-1 overflow-hidden p-6">
        <div className="h-full bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
            ...
            <div className="p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="border border-slate-200 rounded-xl p-6 hover:border-purple-300 transition-colors cursor-pointer group">
                  ...
                  <p className="text-sm text-slate-500 mb-4">Analyzes Identity and Experience. Outputs structured JSON operations for meta tags.</p>
                ...
                <div className="border border-slate-200 rounded-xl p-6 opacity-60">
                  ...
                  <p className="text-sm text-slate-500 mb-4">Executes reading tools on documents to generate executive summaries.</p>
          ...
          {/* Panel: Prompt & Policy */}
          {activePanel === 'prompt & policy' && (
            <div className="p-8 flex flex-col h-full">
              <h2 className="text-lg font-bold text-slate-900 mb-6">Policy, Tool & Prompt Configuration</h2>
              <div className="flex-1 bg-slate-50 rounded-lg border border-slate-200 p-6 flex flex-col items-center justify-center text-slate-500">
                <Settings2 className="h-10 w-10 text-slate-400 mb-4" />
                <p>Select a Capability first to configure its Policy (Tools, Providers) and Prompt context.</p>
              </div>
            </div>
          )}
          ...
          {activePanel === 'suggestions' && (
            <div className="flex flex-col h-full">
              <div className="p-6 border-b border-slate-200 flex justify-between items-center">
                <div>
                  <h2 className="text-lg font-bold text-slate-900">Artifact & Suggestion Viewer</h2>
                  <p className="text-sm text-slate-500">Review atomic operations generated by AI before approval.</p>
                </div>
              </div>
              <div className="flex-1 p-6 bg-slate-50 flex items-center justify-center">
                <div className="max-w-md text-center">
                  ...
                  <h3 className="text-slate-900 font-medium mb-1">No Pending Artifacts</h3>
                  <p className="text-slate-500 text-sm">Execute a capability to generate suggestions or artifacts.</p>
                </div>
              </div>
            </div>
          )}
          ...
          {activePanel === 'history' && (
            <div className="p-8">
              <h2 className="text-lg font-bold text-slate-900 mb-6">Conversation History & Telemetry</h2>
              <div className="space-y-4">
                <div className="p-4 border border-slate-200 rounded-lg bg-white">
                  ...
                  <div className="flex gap-4 text-sm text-slate-500">
                    ...
                    <span>Operation: <strong className="text-purple-600">REPLACE</strong></span>
                  </div>
                </div>
              </div>
            </div>
